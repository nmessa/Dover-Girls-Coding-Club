## Dreamhouse
## Author: nmessa
##We want to take this working program and add something
##to it to make it more interesting.  What can we do?
...
def drawHouseBody():
    p1 = win.getMouse()
    p2 = win.getMouse()
    houseColor = random.choice(COLORS)
    house = Rectangle(p1, p2)
    house.setFill(houseColor)
    house.draw(win)
    return p1, p2
...
def drawRoof():
    ...
    roof.draw(win)
    return p3, p4, p5, roofColor

def drawChimney(roofColor):
    p6 = win.getMouse()
...
#Calculate parmeters based on house body location
